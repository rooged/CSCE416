 ...
 */
public class ConferenceClient implements Runnable
{
	...
	public ConferenceClient(BufferedReader reader, PrintWriter writer)
	{
	...
	{	
		...
		if (args.length != 3) {
			System.out.println("usage: java ConferenceClient <server host> <server port> <client name>");
			System.exit(1);
		}
		...
		try {			
			...
			System.out.println("Connected to server at " + args[0] + ":" + args[1] + ":" + args[2]);
			clientName = args[2];
		}
		catch(Exception e) {
			System.out.println(e);
		...
		try {
			...
			Thread child = new Thread(
					new ConferenceClient(fromUserReader, toSockWriter));
			child.start();
		}
		catch(Exception e) {
			System.out.println(e);
